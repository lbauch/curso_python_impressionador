...
def salvar_banco(dic_avaliacoes):
    with open("reviews.txt", "w", encoding="utf-8") as arquivo:
        for avaliacao in dic_avaliacoes["avaliacoes"]:
            arquivo.write(f"{avaliacao}\n")
    return dic_avaliacoes

runnable_salvar_banco = RunnableLambda(salvar_banco)

chain_global = chain | runnable_salvar_banco | chain_analise

# resposta_analise = chain_analise.invoke({"lista_reviews": resposta})

resposta_analise = chain_global.invoke({"reviews": reviews})
